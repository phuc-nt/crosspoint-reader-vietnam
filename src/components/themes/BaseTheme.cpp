...
void BaseTheme::drawRecentBookCover(GfxRenderer& renderer, Rect rect, const std::vector<RecentBook>& recentBooks,
  ...
  if (hasContinueReading) {
    ...
    const int maxTextWidth = bookWidth - 20;
    auto lines = renderer.wrappedText(UI_12_FONT_ID, lastBookTitle.c_str(), maxTextWidth, 2);

    if (coverRendered) {
      // Draw title and author ABOVE the cover card
      int titleY = bookY - 5;  // Start above the card

      if (!lastBookAuthor.empty()) {
        const auto truncatedAuthor =
            renderer.truncatedText(UI_10_FONT_ID, lastBookAuthor.c_str(), maxTextWidth);
        titleY -= renderer.getLineHeight(UI_10_FONT_ID);
        renderer.drawCenteredText(UI_10_FONT_ID, titleY, truncatedAuthor.c_str());
      }

      // Draw title lines above author (bottom-up)
      for (int i = static_cast<int>(lines.size()) - 1; i >= 0; i--) {
        titleY -= renderer.getLineHeight(UI_12_FONT_ID);
        renderer.drawCenteredText(UI_12_FONT_ID, titleY, lines[i].c_str(), true, EpdFontFamily::BOLD);
      }
    } else {
      // No cover: center title and author within the card (original behavior)
    ...
    // "Continue Reading" label at the bottom of the card
